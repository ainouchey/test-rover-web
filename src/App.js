...
class App extends React.Component {
          ...
          this.showNotification(
            ...
            () => {
              if (event.detail.waiting)
                event.detail.waiting.postMessage({
                  message: "skipWaiting",
                  type: "SKIP_WAITING",
                });
            }
  ...
  async acquireWakeLock(forceReaquire = false) {
    ...
    if (
      ...
      (ls.get("screenOn") !== null ? ls.get("screenOn") : true) &&
      (!this._screenWakeLock || forceReaquire)
    ) {
  ...
  releaseWakeLock(forceRelease = false) {
    ...
    if (
      ...
      ((!this.state.logging && this.state.roverState.status !== 2) ||
        forceRelease)
    )
      this._screenWakeLock.release().then(() => {
  ...
  handleVisibilityChange = () => {
    if (document[hidden]) {
      ...
      if (this.state.isConnected)
        this.state.rover.stopTxNotifications(this.handleRoverTX);
      ...
      if (this.state.isConnected) {
        ...
        this.showNotification(
          "Rover updates and logging are paused while the app is hidden",
          "status-warning",
          5000
        );
        ...
        if (this.state.logging)
          this.startLogging(
            this.state.logging.tableID,
            this.state.logging.interval,
            this.state.logging.targets,
            true
          );
  ...
  */
  addMovingData(item, dataSet) {
    if (item.length === 13) {
      ...
      dataSet.push({
        time: currentTime,
        X: incomingItem.getFloat32(1, true),
        Y: incomingItem.getFloat32(5, true),
        Z: incomingItem.getFloat32(9, true),
      });
    } else {
  ...
  parseMotorControllerStatus(data) {
    ...
    if (status.online === -1) {
      ...
      if (status.error.charAt(0) === ",") status.error = status.error.slice(2);
    }
    return status;
  }
  ...
  intervalSendGamepad = () => {
    if (
      ...
      this.state.roverState.status === 2 &&
      this._gamepadUpdated
    ) {
    ...
    return (
      <Grommet full theme={RoverTheme} themeMode={this.state.themeMode}>
        ...
        >
          <Box width={{ max: "1250px" }} gap="small">
            {this.state.notifications.map((notification) => (
              <StyledNotification
                actionHandle={notification.actionHandle}
                actionText={notification.actionText}
                key={notification.key}
                id={notification.key}
                text={notification.text}
                onClose={notification.closeHandler}
                background={notification.background}
              />
            ))}
          </Box>
        </Layer>
        {this.state.isConnected && this.state.roverState.status === 2 && (
          <GamepadHandler
        ...
        <Box fill="vertical" overflow="auto" align="center" flex="grow">
          <Header // barre en haut avec connected et indicateur
            className="appHeader"
            align="end"
            justify="center"
            pad="medium"
            ...
            <ResponsiveContext.Consumer>
              {(size) => (
                ...
                >
                  ...
                  >
                    {size !== "small" &&
                      size !== "xsmall" && ( // Not connected / connected text
                    ...
                    {this.state.isConnected ? ( // connect / disconnect button (conditional rendering)
                      <Button
                        label="Disconnect"
                  ...
                  >
                    <Collapsible
                      direction="vertical"
                      open={this.state.isConnected}
                    >
                      <Box align="end" justify="center" direction="column">
                        <Heading level="3" margin="none" textAlign="start">
                          {this.state.isConnected
                            ? this.state.rover.getDevice().name
                            : "-"}{" "}
                          {this.state.logging && " [logging]"}
                        </Heading>
                        <Heading level="4" margin="none" textAlign="start">
                          {this.state.roverState &&
                          this.state.roverState.statusMessage
                            ? this.state.roverState.statusMessage
                            : "UNKNOWN"}
                        </Heading>
                      </Box>
                    </Collapsible>
                    <StatusGoodSmall // indicateur cercle couleur
                      color={
                        this.state.roverState &&
                        this.state.roverState.statusColor
                          ? this.state.roverState.statusColor
                          : "status-unknown"
                      }
                      size="large"
          ...
          <Box // main section of page (below header)
            ...
            <Tabs justify="center" flex>
              ...
              <StyledCard title="System" wide>
                    ...
                    <StateBox
                      ...
                      name="On time"
                      value={!this.state.roverState.ontime && "-"}
                    >
                      {this.state.roverState.ontime && (
                        <Clock
                          type="digital"
                          time={this.state.roverState.ontime}
                        />
                      )}
                    </StateBox>
                  </StyledCard>
